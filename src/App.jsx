...
export default function App() {
  ...
  const handleStartQuiz = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("השרת לא מגיב. ודא שהקובץ api/claude.js קיים ב-GitHub.");
    ...
    <div style={{ direction: "rtl", minHeight: "100vh", background: "#0f172a", color: "#fff", padding: 20, fontFamily: "'Varela Round', sans-serif" }}>
      {loading ? (
        ...
        <div style={C.card}>
          ...
          <input style={C.inp} placeholder="שם משפחה" onChange={e => setFamily({name: e.target.value, members: [{name: "אבא", age: 40}, {name: "ילד", age: 8}]})} />
          <button style={C.btnP} onClick={() => setScreen("home")}>כניסה</button>
        </div>
      ) : screen === "home" ? (
        <div style={C.card}>
          <h2>שלום משפחת {family?.name}!</h2>
          <input style={C.inp} placeholder="נושא לחידון..." value={topic} onChange={e => setTopic(e.target.value)} />
...
function QuizScreen({ quizData, onFinish }) {
  ...
  return (
    <div style={C.card}>
      <div style={{ color: "#a78bfa", marginBottom: 10 }}>תור של: {q.m}</div>
      <h2 style={{ marginBottom: 20 }}>{q.q}</h2>
      <div style={{ display: "grid", gap: 12 }}>
        {q.o.map((option, i) => (
          <button key={i} onClick={() => handleChoice(i === q.a)} style={{ padding: "14px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 12, color: "#fff", cursor: "pointer", textAlign: "right" }}>
            {option}
          </button>
        ))}
      </div>
